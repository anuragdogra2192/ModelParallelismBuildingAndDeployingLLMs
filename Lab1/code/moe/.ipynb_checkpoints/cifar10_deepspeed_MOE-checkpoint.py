...
writer = SummaryWriter('/dli/nemo/cifar10_DS' + str(args.profile_name))
with ExitStack() as stack:
    if args.profile_execution and torch.distributed.get_rank()==0:
        prof = stack.enter_context(torch.profiler.profile(schedule=torch.profiler.schedule(wait=1, warmup=1, active=2, repeat=1),
                        on_trace_ready=torch.profiler.tensorboard_trace_handler('/dli/nemo/cifar10_' + str(args.profile_name)+ str(torch.distributed.get_rank())),
                        record_shapes=True,
                        profile_memory=True,
                        with_stack=True,
                        with_flops=True
